...
export default function GamePage() {
  ...
  if (phase === "already_played") {
    ...
    const shareText = result ? encodeURIComponent(`I just completed Arc Trivia 1.0 and earned the "${alreadyTier?.name}" SBT! 🎮\n\nAre you an Arc Maxi? Go Try it now`) : "";
  ...
  if (phase === "finished" && result && tier) {
    const shareText = encodeURIComponent(`I just completed Arc Trivia 1.0 and earned the "${tier.name}" SBT! 🎮\n\nAre you an Arc Maxi? Go Try it now`);
    const shareUrl = encodeURIComponent(`${process.env.NEXT_PUBLIC_BASE_URL ?? ""}/share/${address}`);
    return (
      <Shell>
        <div className="max-w-sm w-full text-center space-y-5">
